...
import PyPDF2
import json
from tools.GPTImg2Text import GPT_output
...
class FastApiApp:
    ...
    def setup_routes(self):
        ...
        async def chat(request: Request):
            ...
            slide_text = ""
            vision_text = ""
            vision_info = json.load("vision_outputs.json")[slide_number]

            #Extract the vision input for the slide and put it in a proper format
            for idx, exp in enumerate(vision_info):
                vision_text += f"Image {idx}:\n"
                vision_text += exp + "\n\n\n"
            ...
            system_prompt += f"\n\nVISUAL LECTURE CONTENT ON SLIDE:\n\n{vision_text}"
        ...
        async def publish_pdf(request: Request):
            ...
            try:
                results = GPT_output(self.pdf_path)
                #Parse the results and write it to local text file
                vision_output = {}
                with open("claude_text_input.txt","wb") as f:
                    for page_number in results.keys():
                        f.write(results[page_number]["Text Input"] +"#*$")

                        #Clean up the output for Image Explanation
                        img_exp_page = []
                        for exp in results[page_number]["Image Explantion"]:
                            img_exp = json.load(exp)["answer"]
                            img_exp_page.append(img_exp)

                        #Add the vision output to a dictionary
                        vision_output[page_number] = img_exp_page

                #Dump it to drive
                with open("vision_output.json","wb") as f: 
                    json.dumps(vision_output,f)
